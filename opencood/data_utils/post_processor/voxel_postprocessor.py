...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process_save(self, data_dict, output_dict):
        """
        Process the outputs of the model to 2D/3D bounding box.
        Step1: convert each cav's output to bounding box format
        Step2: project the bounding boxes to ego space.
        ...
        for cav_id in output_dict.keys():
            # print(cav_content['transformation_matrix'])
            # import time
            # time.sleep(1000)
            # assert cav_id in output_dict
            # print(output_dict[cav_id].keys())['processed_lidar', 'object_bbx_center', 'object_bbx_mask', 'record_len', 'label_dict', 'object_ids', 'pairwise_t_matrix', 'lidar_pose_clean', 'lidar_pose', 'anchor_box', 'cav_id_list', 'timestep', 'scenario_folder', 'origin_lidar', 'transformation_matrix', 'transformation_matrix_clean', 'sample_idx']
            # print(cav_content.keys())['cls_preds', 'reg_preds', 'dir_preds'])
            ...
            # classification probability
            prob = output_dict[cav_id]['cls_preds']
            # print(prob.shape)[1, 2, 100, 352]
            prob = F.sigmoid(prob.permute(0, 2, 3, 1))
            # print(prob.shape)([1, 100, 352, 2])
            prob = prob.reshape(1, -1)
            # print(prob.shape)[1, 70400])

            # regression map
            reg = output_dict[cav_id]['reg_preds']
            # print(reg.shape)[1, 14, 100, 352]
            # print(anchor_box.shape)[100, 352, 2, 7]
            # convert regression map back to bounding box
            if len(reg.shape) == 4: # anchor-based. PointPillars, SECOND
                batch_box3d = self.delta_to_boxes3d(reg, anchor_box)
            else: # anchor-free. CenterPoint
                batch_box3d = reg.view(1, -1, 7)
            # print(batch_box3d.shape)[1, 70400, 7]
            mask = \
                torch.gt(prob, self.params['target_args']['score_threshold'])
            # print(mask.shape)[1, 70400]
            mask = mask.view(1, -1)
            # print(mask.shape)1, 70400
            mask_reg = mask.unsqueeze(2).repeat(1, 1, 7)
            # print(mask_reg.shape)1, 70400, 7

            # during validation/testing, the batch size should be 1
            assert batch_box3d.shape[0] == 1
            boxes3d = torch.masked_select(batch_box3d[0],
                                          mask_reg[0]).view(-1, 7)
            # print(boxes3d.shape)[108, 7]
            scores = torch.masked_select(prob[0], mask[0])
            # print(scores.shape)108
            ...
            if len(boxes3d) != 0:
                ...
                boxes3d_corner = \
                    box_utils.boxes_to_corners_3d(boxes3d,
                                                  order=self.params['order'])
                # print(boxes3d.shape)torch.Size([108, 7])
                # print(boxes3d_corner.shape)torch.Size([108, 8, 3]
                # STEP 2
                # (N, 8, 3)
                projected_boxes3d = \
                    box_utils.project_box3d(boxes3d_corner,
                ...
                # print(projected_boxes3d.shape)[109, 8, 3]
                # import time
                # time.sleep(1000)
                # convert 3d bbx to 2d, (N,4)
                projected_boxes2d = \
                ...
                pred_box3d_np, mask = box_utils.mask_boxes_outside_range_numpy(pred_box3d_np,
                                                            ...
                                                            order=None,
                                                            return_mask=True)
                single_pred.append(torch.from_numpy(pred_box3d_np))
                scores = scores[mask]
                boxes3d = boxes3d[mask]
                single_pose.append(boxes3d.cpu())
                single_score.append(scores.cpu())
                
                pred_box2d_list.append(boxes2d_score)
                pred_box3d_list.append(projected_boxes3d)
                
        # split_path = data_dict['ego']['scenario_folder'][0].split('/')
        # print(split_path)
        save_path = os.path.join('opencood/logs/dairv2x_point_pillar_lidar_max_2023_04_13_13_34_49', "train", "single_pred_gt",  data_dict['ego']['scenario_folder'][0])
        # save_path = os.path.join('opencood/logs/v2xsim_max_2023_04_18_05_13_50', "train", "single_pred_pose")
        ...
        if not os.path.exists(save_path):
            try:
                ...
                pass
        save_path = os.path.join(save_path, data_dict['ego']['timestep'][0]+".pt")
        save_dict = {"pred": single_pred, "scores": single_score, "transform": data_dict['ego']['pairwise_t_matrix'].cpu(), "pose": single_pose, 'lidar_pose': data_dict['ego']['lidar_pose'].cpu()}
        ...
        assert scores.shape[0] == pred_box3d_tensor.shape[0]
        return pred_box3d_tensor, scores, save_dict, save_path

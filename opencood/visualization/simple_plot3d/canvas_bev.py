...
class Canvas_BEV_heading_right(object):
    ...
    def draw_lines(self,
        ...
        if colors is None:
            colors = np.full(
                (len(start_xyz), 3), fill_value=255, dtype=np.uint8)
        elif isinstance(colors, tuple):
            assert len(colors) == 3
            colors_tmp = np.zeros((len(start_xyz), 3), dtype=np.uint8)
            colors_tmp[..., :len(colors)] = np.array(colors)
            colors = colors_tmp
        elif isinstance(colors, np.ndarray):
            assert len(colors) == len(start_xyz)
            colors = colors.astype(np.uint8)
        else:
            raise Exception(
                "colors type {} was not an expected type".format(type(colors)))
        
        start_pts_xy, start_pts_valid_mask = \
            self.get_canvas_coords(start_xyz)
        end_pts_xy, end_pts_valid_mask = \
            self.get_canvas_coords(end_xyz)
